...
export class CompleteOrderUseCase {
  ...
  async execute(id: string): Promise<ServiceOrderResponseDto> {
    this.logger.log(`Completing service order: ${id}`);
    ...
    // Lidos antes da transição: `order` é mutado por `complete()`,
    ...
    this.logger.log(`Service order ${id} transitioned to COMPLETED`);
